...
public class FormulaModSimplex01_2D : FormulaMod
{
    ...
    public override string GenerateCode(HashSet<int> vars, StringBuilder builder)
    {
        if (false == vars.Contains(VarIndex))
        {
            ...
            builder.AppendLine($"            {PickCode(2, m_offsetX, vars, builder)} + {val0} * {PickCode(4, m_mulX, vars, builder)},");
            builder.AppendLine($"            {PickCode(3, m_offsetY, vars, builder)} + {val1} * {PickCode(5, m_mulY, vars, builder)});");
        }
