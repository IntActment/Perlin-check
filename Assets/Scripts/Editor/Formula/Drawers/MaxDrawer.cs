...
public class MaxDrawer : BaseDrawer
{
    public override string HelpMessage => mod.Inputs[1].Link == null
        ? m_helpMessage1
        : m_helpMessage2;

    private const string m_helpMessage1 = "[<color=orange>max</color>] returns maximum value.\n" +
        "Underlying formula will be like\n" +
        "        res = max(a, b)\n" +
        "where 'a' is input slot parameter and\n" +
        "      'b' is configurable field parameter.";

    private const string m_helpMessage2 = "[<color=orange>max</color>] returns maximum value.\n" +
        "Underlying formula will be like\n" +
        "        res = max(a, b)\n" +
    ...
    public MaxDrawer(FormulaModMax mod)
        : base(mod) { }

    static MaxDrawer()
    {
        Register(typeof(FormulaModMax), mod => new MaxDrawer((FormulaModMax)mod));
    }
    ...
    protected override void DrawBodyGUI(InputState state)
    {
        ...
        FormulaModMax m = (mod as FormulaModMax);
